...
# taki zapis _rows = to pole prywatne
    ...
    def to_json(self):
        json_dict = {}
        json_dict['columns'] = self.column_names
        json_dict['rows'] = self._rows
        return json.dumps(json_dict)

        # lub data = {'columns': self.column_names, 'rows': self._rows}
        # return json.dumps(data)

    @staticmethod
    def from_json(json_data):
        json_dict = json.loads(json_data)
        table = TabularData(json_dict['columns'])
    ...
    def to_json_file(self, output_file):
        ...
        json_dict['columns'] = self.column_names
        json_dict['rows'] = self._rows
        return json.dump(json_dict, output_file)
